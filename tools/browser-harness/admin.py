...
from _compat import (
    _IS_WIN, SOCK_PATH, LOG_PATH, PID_PATH, VERSION_CACHE,
    try_connect_daemon, make_client_socket, _remove_port,
)
...
def _paths(name):
    n = name or NAME
    if _IS_WIN:
        from _compat import _TEMP
        return str(_TEMP / f"bu-{n}.port"), str(_TEMP / f"bu-{n}.pid")
    return f"/tmp/bu-{n}.sock", f"/tmp/bu-{n}.pid"


def _log_tail(name):
    p = LOG_PATH if not name else (LOG_PATH.parent / f"bu-{name}.log")
    try:
...
def daemon_alive(name=None):
    return try_connect_daemon() if not name else try_connect_daemon()
...
def ensure_daemon(wait=60.0, name=None, env=None):
    ...
    for attempt in (0, 1):
        ...
        p = subprocess.Popen(
            ...
            # start_new_session is Unix-only; omit on Windows
            **({"start_new_session": True} if not _IS_WIN else {}),
        )
    ...
    if pid:
        for _ in range(75):
            try:
                ...
                # Windows may raise OSError instead of ProcessLookupError
                break
        else:
            try:

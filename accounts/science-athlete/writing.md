# Writing Rules: Science Athlete Account

## Core Voice

Write for athletes who understand their sport but have never had the hidden
science explained in their own language.

The copy should feel like a coach who knows the sport, understands the science,
and can explain both without sounding like a textbook.

## Base Tone

- Practical before academic.
- Sport language before science language.
- Clear before clever.
- Specific before dramatic.
- Researched, but not dense.
- Slightly confrontational only when naming a real training mistake.

The reader should not need to reread a slide to understand the point.

## Writing Promise

Each post should make a familiar athletic problem feel newly explainable.

Examples:

- Soccer: "I look clean in cone drills, but I feel stiff in games."
- Basketball: "My handles work alone, but I lose them when a defender reaches."
- Boxing: "I look fast on pads, but I freeze when someone punches back."

The post explains why that happens, then gives a fix the athlete can try.
...
Use **one unfamiliar science term per slide maximum**.

If the term is not common, explain it in brackets the first time it appears.

Good:

> The amygdala [the brain's threat-detection system] reads pressure as danger.
...
Stats can be used in drafts, but final stats must be source-backed or rewritten.

- Use stats only when the number sharpens the argument.
- Place the stat near the beginning of the explanation when it frames the slide.
- If a number is invented for draft tone, mark it `[VERIFY]`.
- Never leave made-up percentages in final copy.
- Prefer real source phrasing over fake precision.
...
> Players who scan before receiving make 38% more forward actions. [VERIFY]

## Slide Body Formula

Use this as the default internal structure. The final copy does not need visible
...
version matters, fold it into **the fix:** in one short sentence.

## Compressed Carousel Copy Rule
...
w(inner) should appear only when the post has earned the mental-reps bridge.

Natural product contexts:
...
- game picture replay
- match-day prep
- learning the read when the body cannot take more reps
...
- w(inner) fixes confidence
- w(inner) guarantees performance
- w(inner) cures anxiety
- w(inner) replaces coaching
- w(inner) makes you elite
...
- [ ] Would an athlete in this sport understand the first sentence instantly?
- [ ] Does the slide use one main idea?
- [ ] If a science term appears, is it explained in brackets?
- [ ] Is any stat verified or marked `[VERIFY]`?
- [ ] Does the fix feel doable today?
- [ ] Does the fix include why it works?
- [ ] Does the product mention feel native, or should it be removed?
- [ ] Does the source support the claim without overstating it?
- [ ] Any filler, hype, or generic motivation to cut?

"""A3 quality-gate renderer for athlete-max — attempt 006 soccer body slide.

Structural change: verified_byhumans Post 1 listing format.
- Label: "what to avoid:" (small regular)
- Title: short topic name (large bold, like "mouth breathing while distracted")
- Explanation with contrarian hook embedded
- Photo centered with good vertical spacing
- Fix + payoff embedded
- Source
...
# Explanation text bottom is ~364 (5 lines at 28px + 4 gaps at 8px, starting at y=220)
# Lower section is 244px (7 lines at 28px + 6 gaps at 8px)
# Source at y=1280, need lower end <= 1240 (40px buffer)
# Equal gap G: 364 + G + 450 + G + 244 <= 1240
# 1058 + 2G <= 1240, 2G <= 182, G <= 91. Use G = 80.
FRAME_Y = 444  # explanation text bottom (364) + 80px gap
FRAME_BOTTOM = FRAME_Y + FRAME_H  # 894
...
def main():
    ...
    # Label — "what to avoid:" (small regular, like verified_byhumans Post 1)
    label_text = "what to avoid:"
    check_width(draw, label_text, F_LABEL, "label")
    center_text(draw, label_text, 95, F_LABEL, BLACK)

    # Title — the thing to avoid (large bold, completing the label)
    title_text = "losing your late-game touch"
    check_width(draw, title_text, F_TITLE, "title")
    center_text(draw, title_text, 145, F_TITLE, BLACK)
    ...
    # Reference photo frame — centered
    draw_placeholder_frame(draw)

    # Lower section — fix + payoff embedded, 80px gap after frame (equal to gap above)
    lower_start = FRAME_BOTTOM + 80  # 894 + 80 = 974

    fix_paragraph = [
        [("the fix:", F_BODY_BOLD, BLACK),
         (" after your main session, spend 10 minutes on wall", F_BODY, BLACK)],
        [("passes alone. ", F_BODY, BLACK),
         ("constraint:", F_BODY_BOLD, BLACK),
         (" every third touch must be one-timed into a", F_BODY, BLACK)],
        [("shoebox-sized target you tape to the wall. the smaller target", F_BODY, BLACK)],
        [("forces your motor cortex to maintain precision, so your touch", F_BODY, BLACK)],
        [("stays clean in minute 80 when the defender expects you to lose", F_BODY, BLACK)],
        [("the ball because your brain stays sharp under late-game pressure.", F_BODY, BLACK)],
    ]
